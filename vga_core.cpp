 ...
 * CARD core methods
 *********************************************************************/
CardCore::CardCore(uint32_t core_base_addr) {
   base_addr = core_base_addr;
}
CardCore::~CardCore() {
}
...
void CardCore::wr_char(uint8_t x, uint8_t y, int ch) {
   uint32_t ch_offset;
   uint32_t data;

   ch_offset = (y << 5) + (x & 0x01f);
   data = (uint32_t) ch;
   io_write(base_addr, ch_offset, data);
   return;
}

void CardCore::clr_screen() {
   int x, y;
